            ...
            if ("alipay".equals(configService.getValueByName("alipay"))) {
                ...
                for (Order order : orders) {
                    ...
                    if (("TRADE_SUCCESS".equals(response.getTradeStatus()) || "TRADE_FINISHED".equals(response.getTradeStatus()))) {
                        ...
                        if (id.startsWith("p_")) {
                            Order existOrder = orderService.getOrderByOrderId(id.split("_")[1]);
                            ...
                            log.info("查单成功,开始处理: {}", response.getOutTradeNo());
                            redisService.del("panel::user::" + order.getUserId());
                            existOrder.setPayAmount(new BigDecimal(response.getTotalAmount()));
                            // 查询用户当前套餐
                            ...
                            // 更新订单
                            if (orderService.updateFinishedOrder(existOrder.getPayAmount(), "支付宝", response.getBuyerUserId(), isNewPayer, response.getTradeNo(), response.getSendPayDate(), PayStatusEnum.SUCCESS.getStatus(), existOrder.getId())) {
                                existOrder.setPayType("支付宝");
                                ...
                                if (ObjectUtil.isNotEmpty(user.getParentId())) {
                                    ...
                                    if (ObjectUtil.isNotEmpty(inviteUser) && inviteUser.getClazz() > 0) {
                                        ...
                                        BigDecimal commission = existOrder.getPayAmount().multiply(inviteUser.getInviteCycleRate()).setScale(2, BigDecimal.ROUND_HALF_UP);
                                        // 判断是循环返利还是首次返利
